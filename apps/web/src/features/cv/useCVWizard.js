// features/cv/useCVWizard.js
// Estado, efectos y handlers del wizard de la Factoría CV, extraídos VERBATIM
// desde el orquestador (pages/CVDesdeCero.jsx). La persistencia delicada (autosave
// con debounce, flush en unmount vía refs, sessionStorage['cv_draft_${user.id}'] y
// job_search_profile.cv_borrador) se movió SIN tocar shapes (regla CLAUDE.md).
// El orquestador llama useCVWizardState() y provee el resultado por CVWizardContext.
...
export function useCVWizardState() {
  ...
  // Derivado: % de llenado del wizard (antes computado en el render del orquestador)
  const pctLlenado = calcularLlenado(datos)

  // ── API expuesto por context (solo lo que consumen los componentes) ──────────
  return {
    // contexto base
    modoForzado, user, navigate, fileRef,
    // estado
    showCancelModal, borradorPendiente, pasoActual, datos, generando, cvGenerada,
    error, inicializando, ultimoGuardado, extrayendo, analisis, cvMismatch,
    cvFileName, alertaExistente, modoSeleccion, optimizandoResumen, resumenSugerido,
    resumenBloqueado, cvResumenOriginal, ofertaValorGerente, fusionando, errorFusion,
    resumenFusionSugerido, cvIdioma, alertaIdioma, expSugeridas, expOptimizando,
    expMejoradas, alertaPlaceholder, borradorFinal, scoreAntes, nuevaHab,
    // setters consumidos por componentes
    setShowCancelModal, setBorradorPendiente, setPasoActual, setDatos, setCvGenerada,
    setAnalisis, setCvMismatch, setCvPending, setCvFileName, setAlertaExistente,
    setModoSeleccion, setResumenSugerido, setResumenBloqueado, setResumenFusionSugerido,
    setAlertaIdioma, setExpSugeridas, setAlertaPlaceholder, setBorradorFinal, setNuevaHab,
    // helpers de estado
    upDatos, upExp, addExp, delExp, upEdu, addEdu, delEdu, togHab, addHab, togIdm, upNivIdm,
    // handlers
    extraerCV, handleFusionarResumen, handleOptimizarResumen, handleOptimizarExp,
    aplicarSugerenciaExp, rechazarSugerenciaExp, iniciarGenerarCV, generarCV,
    confirmarYGuardar, continuarBorrador, descartarYEmpezar,
    // derivado
    tipsPorPaso, pctLlenado,
  }
}
